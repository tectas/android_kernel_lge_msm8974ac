...
#if defined(CONFIG_MACH_MSM8974_G3_SPR_US) || defined(CONFIG_MACH_MSM8974_G3_ATT) || defined(CONFIG_MACH_MSM8974_G3_TMO_US)
#define LM3697_MAX_BRIGHTNESS		1892
#else
#define LM3697_MAX_BRIGHTNESS		2027
#endif
...
static int bl_max_value = LM3697_MAX_BRIGHTNESS;
/*
 * struct lm3697_bl
...
static ssize_t show_bl_max(struct device *dev, struct device_attribute *attr, char *buf)
{
	int r = 0;

	r = snprintf(buf, PAGE_SIZE, "max bl_map value is %d\n", bl_max_value);

	return r;
}

static ssize_t store_bl_max(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct lm3697_bl* lm3697_bl = bl_get_data(lm3697_device);
	struct lm3697_backlight_platform_data* pdata = lm3697_bl->bl_pdata;
	int max_level = 255;
	...
	node_value = simple_strtoul(buf, NULL, 10);
	...
	pr_info("%s: node_value is = %d\n", __func__,node_value);

	pdata->blmap[max_level] = bl_max_value;

	if(lm3697_bl->bl_dev->props.brightness == max_level) {
		lm3697_bl_set_brightness(lm3697_bl, bl_max_value);
	}

	return count;
}

DEVICE_ATTR(lm3697_bl_max, 0644, show_bl_max,store_bl_max);
			   ...
			   const struct i2c_device_id *id)
{
	...
	device_create_file(&cl->dev, &dev_attr_lm3697_bl_max);
