                     ...
                     uint16 devid)
{
	...
	dhd_check_module_vid((void *)bus, osh);
...
#if defined(CUSTOMER_HW10) && defined(SUPPORT_MULTIPLE_MODULE_VID)
int dhdsdio_cis_read(void *bus, uint fn, uint8 *cis_buf, int cis_buf_len)
{
	int ret = BCME_OK;
	struct dhd_bus *dhd_bus = bus;

	bzero(cis_buf, cis_buf_len);

	if ((ret = bcmsdh_cis_read(dhd_bus->sdh, fn, cis_buf, cis_buf_len))) {
		DHD_ERROR(("%s: fn %d cis_buf read ret %d\n", __func__, fn, ret));
	}
